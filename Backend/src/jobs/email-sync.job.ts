  ...
  private async runInitialSync(account: {
    ...
    try {
      ...
      if (account.provider === 'gmail' && account.oauthTokens) {
        // Gmail: Fetch initial emails and set up push notifications
      ...
      // For Gmail: Set up push notifications after initial sync
      if (account.provider === 'gmail' && account.oauthTokens) {
        try {
          ...
          logger.info(`📡 Gmail push notifications enabled for ${account.emailAddress}`);
        } catch (pushError) {
